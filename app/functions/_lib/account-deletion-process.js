...
import { cancelBillingBeforeDeletion } from './account-deletion-billing.js';
...
export async function processAccountDeletion(env, {uid,email=null}) {
  ...
  const admission=await beginDeletionAdmission(env,{uid,email});
  ...
  async function billing() {
    ...
    await cancelBillingBeforeDeletion(env,{uid,user,email:job.email});
    billingChecked=true;
  }
  try {
    ...
    if(job.phase==='billing_verified') {
      ...
      if(exists) {
        ...
        if(!billingChecked) await billing();
        stage='identity_unconfirmed';identityRemoved=null;
