...
const NAVIGATION_CONFIG = {
  ...
  visitor: {
    ...
    cta: { text: 'Start Free Trial', href: '/pricing/', isCTA: true }
  },
  ...
  free: {
    navItems: [
      ...
      { text: 'Blog', href: '/index.html#blog' },
      { text: 'Resume Feedback', href: '/resume-feedback-pro/', locked: true },
      { text: 'Interview Questions', href: '/interview-questions/', locked: true }
    ],
    userNav: {
      cta: { text: 'Upgrade', href: '/pricing/', isCTA: true },
      menuItems: [
        { text: 'Account', href: '/account-setting/' },
  ...
  trial: {
    navItems: [
      ...
      { 
        ...
        isDropdown: true,
        items: [
          { text: 'Resume Feedback', href: '/resume-feedback-pro/' },
        ]
      },
      { 
        text: 'Interview Prep',
        isDropdown: true,
        items: [
          { text: 'Interview Questions', href: '/interview-questions/' },
        ]
      }
    ],
    userNav: {
  ...
  pro: {
    navItems: [
      ...
      { 
        ...
        items: [
          { text: 'Resume Feedback', href: '/resume-feedback-pro/' },
          { text: 'Resume Rewrite', href: '/resume-feedback-pro/#rewrite' },
          { text: 'Cover Letter', href: '/cover-letter-generator/' },
        ]
      },
      { 
        text: 'Interview Prep',
  ...
  premium: {
    navItems: [
      ...
      { text: 'LinkedIn Optimizer', href: '/linkedin-optimizer/' }
    ],
    userNav: {
      menuItems: [
        { text: 'Account', href: '/account-setting/' },
    ...
    ">
      ...
      <div style="display: flex; gap: 0.75rem; justify-content: center;">
        <button id="upgrade-cancel-btn" style="
          ...
          font-weight: 600;
        ">Cancel</button>
        <a href="/pricing/?plan=${targetPlan}" style="
          background: #00E676;
          color: white;
          border: none;
          padding: 0.75rem 1.5rem;
...
function updateNavigation() {
  ...
  // --- Link helper: normalize all links to safe absolute or hash URLs ---
  const updateLink = (linkElement, href) => {
    ...
    // If this looks like an internal path without protocol or hash, force a single leading slash
    const isHash = finalHref.startsWith('#');
    const isProtocol = /^(https?:)?\/\//i.test(finalHref);
    const isMailOrTel = /^(mailto:|tel:)/i.test(finalHref);
    if (!isHash && !isProtocol && !isMailOrTel) {
      // Avoid accidental protocol-relative URLs like //pricing/
