...
Review follow-up: withdrawing consent now preserves the existing GA script and runtime under the disable flag. Re-grant reuses that instance. A regression check tracks cumulative script insertions across three revoke/re-grant cycles, one configuration and one page view. Clarity remains an open acceptance item.
