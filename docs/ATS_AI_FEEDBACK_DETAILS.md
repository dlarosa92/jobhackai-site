...
// Get ATS score (canonical job title used for both calls)
showLoadingState('Calculating your ATS score…');
const canonicalJobTitle = getCanonicalJobTitle();
const scoreResult = await getAtsScore(uploadResult.resumeId, canonicalJobTitle);

// ... display initial scores ...

// Get feedback (if plan allows) and only refresh textual tips
const plan = getCurrentUserPlan();
if (['trial', 'essential', 'pro', 'premium'].includes(plan)) {
  try {
    showLoadingState('Optimizing for ATS compliance…');
    const feedbackResult = await getResumeFeedback(uploadResult.resumeId, canonicalJobTitle);
    updateFeedbackGrid(feedbackResult.roleSpecificFeedback);
    applyFeedbackTipsToRubric(feedbackResult.atsRubric);
```
...
// Load existing rule-based scores generated by /api/ats-score
const ruleBasedScores = await loadRuleBasedScoresFromSession(resumeId);
```
