...
function clearAuthCookies() {
  try {
    ...
    const maxAge = 60 * 60 * 24 * 30; // 30 days
    const secure = '; Secure';
    // Set jhai_auth=0 as explicit logout signal instead of deleting.
    // Marketing site reads this to distrust stale URL auth handoffs.
    document.cookie = `jhai_auth=0; domain=${domain}; path=/; max-age=${maxAge}; SameSite=Lax${secure}`;
    document.cookie = `jhai_name=; domain=${domain}; path=/; max-age=0`;
